...
<div class="text-center">
    ...
    <p class=" font-extrabold text-gray-300 2xl:text-2xl xl:text-xl lg:text-xl text-md text-center mx-4">
        This page will available timing of the venue based on the week and semester indicated.
    </p>

    <div class="mt-5">
        <p class=" font-semibold text-gray-300 2xl:text-2xl xl:text-xl lg:text-xl text-md text-center mx-4">
            Start using by indicating the semester and the week before clicking the 'Retrive Venue' button.
        </p>
        <p class=" font-semibold text-gray-300 2xl:text-2xl xl:text-xl lg:text-xl text-md text-center mx-4">
            The venue that is available to use for the semester will then be shown.
        </p>
    </div>
    ...
    <button class="mt-10 py-2.5 px-5 mb-2 text-sm bg-gradient-to-r from-pink-600 to-sky-600 rounded-lg border-none text-gray-50 font-bold hover:from-sky-600 hover:to-teal-600" on:click={findVenue}>Retrive Venue</button>

    {#if (error_message_no_input != "") && (error_message_no_input != undefined)}
        <p class="2xl:text-3xl 2xl:mt-10 xl:text-3xl xl:mt-10 lg:text-2xl lg:mt-8 md:text-2xl mt-4 text-xl text center text-red-500">{error_message_no_input}</p>
    {/if}
        ...
        <div class="grid 2xl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 grid-cols-1 2xl:mx-10 xl:mx-10 lg:mx-5 mx-2 gap-y-10">
            ...
            <div class="mx-4 h-96 overflow-y-auto">
                {#each entries as [key, value]}
                    ...
                    <div class="text-left text-slate-200 2xl:text-2xl xl:text-2xl lg:text-2xl text-xl border-b-2 border-slate-500 mx-2 mb-2 mt-5">{key}</div>
                        <div class="grid grid-cols-4 mt-5">
                            {#each value as [startTime, endTime]}
                                <div class="text-slate-200 text-md text-left mx-2">{startTime}-{endTime}</div>
                            {/each}
                        </div>
                    {/if}
                ...
                {:else if loading == false && !completed_process}
                    <p class="2xl:text-3xl xl:text-3xl lg:text-2xl text-xl my-40 text-center text-sky-500">Please select the venue to see the time slots.</p>
                {/if}
